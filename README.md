...
- CloudFormation (CFn): インフラストラクチャをコード化（IaC）し、リソースの自動化と管理を効率化するスキルを習得。

- IAM (Identity and Access Management): ユーザーやグループ、ロールを利用したアクセス制御とセキュリティ設定の管理。

- Route 53: ドメイン名の管理と DNS 設定による、高可用性のシステム構築。
...
- RDS (Relational Database Service): リレーショナルデータベースのプロビジョニング、パフォーマンス最適化、セキュリティ設定。

- S3 (Simple Storage Service): 大量のデータ保存とアクセス管理を行い、バックアップと可用性の確保。

- VPC (Virtual Private Cloud): パブリックとプライベートサブネット、ルートテーブル、セキュリティグループを使った安全なネットワークの設計と管理。

- NAT Gateway: プライベートサブネットから外部インターネットへのセキュアな通信を確立。

- VPC エンドポイント: VPC 内でのサービス間通信を最適化し、インターネットを介さないセキュアな接続を構築。

- EIP (Elastic IP Address): 固定 IP アドレスを利用した安定したインフラ環境の提供。

- SSM (Systems Manager): EC2 インスタンスへのリモート管理と自動化されたメンテナンスの実行。

- CLI (Command Line Interface): AWS リソースの管理をスクリプト化し、効率的な操作を実現。

- Secrets Manager: データベースや API キーなどの機密情報を安全に管理し、自動的なローテーションを実施。

- Aurora:Amazon Aurora のセットアップは理解していますが、Aurora クラスタの自動フェイルオーバーやリードレプリカの管理に関しては、さらに学習が必要です
